...
/**
 * Integration tests for RuleEngine that address User Story 3: "As a homeowner I want an automation
 * rule that, after 11pm, turns on hallway lights when motion is detected and sends me an alert so
 * that I can safely move at night."
 */
public class RuleEngineIntegrationTest {
  ...
  class NightMotionDetection {
    ...
    void testNightMotionDetection() throws RuleConflictException, SceneExecutionException {
      ...
      ruleEngine.handleEvent("motion_detected", "Hallway Camera", LocalTime.of(0, 0)); // Midnight
    ...
    void testMultipleMotionSensors() throws RuleConflictException, SceneExecutionException {
      ...
      // Test hallway camera motion at midnight
      ruleEngine.handleEvent("motion_detected", "Hallway Camera", LocalTime.of(0, 0)); // Midnight
      assertTrue(hallwayLight.isOn());
      ...
      ruleEngine.handleEvent("motion_detected", "Front Door Camera", LocalTime.of(1, 0)); // 1 AM
      assertTrue(hallwayLight.isOn());
  ...
  class TimeBasedRuleEvaluation {
    ...
    void testOvernightTimeWindow() throws RuleConflictException {
      ...
      // Test that rule was added successfully during active hours
      assertDoesNotThrow(() -> ruleEngine.handleEvent("motion_detected", "Hallway Camera", LocalTime.of(2, 0))); // 2 AM
    }
  ...
  class PerformanceAndTiming {
    ...
    void testSceneExecutionTiming() throws RuleConflictException, InterruptedException {
      ...
      assertTrue(
          executionTime < 2000,
          "Scene execution took " + executionTime + "ms, should be under 2000ms");

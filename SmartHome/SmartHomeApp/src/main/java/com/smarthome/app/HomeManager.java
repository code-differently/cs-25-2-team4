...
public class HomeManager {
    ...
    public boolean addRoom(Room room) { return room != null && rooms.add(room); }
    public boolean deleteRoom(Room room) { return room != null && rooms.remove(room); }
    public Set<Room> getRooms() { return rooms; }

    public boolean addDevice(Device device, String roomName) {
        if (device == null || roomName == null) return false;
        Room room = getRoomByName(roomName);
        if (room == null) { room = new Room(roomName); rooms.add(room); }
    ...
    public Device getDeviceByName(String name) {
        ...
        for (Room room : rooms) {
            for (Device device : room.getDevices()) {
                String n = device.getDeviceName();
                if (n != null && n.equalsIgnoreCase(name)) return device;
            }
        }
        return null;
    }

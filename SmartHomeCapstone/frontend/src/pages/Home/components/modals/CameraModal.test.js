...
describe("CameraModal Component", () => {
  ...
  describe("Rendering", () => {
    it("renders modal with device name", () => {
      ...
      // Assert
      expect(
        screen.getByText(/front door camera — camera/i),
      ).toBeInTheDocument();
    });
    ...
    it("renders camera feed image", () => {
      ...
      expect(["camera.gif", "test-file-stub"]).toContain(cameraImage.getAttribute("src"));
    });
  ...
  describe("Edge Cases and Robustness", () => {
    ...
    it.each([["onClose"], ["onToggle"], ["onRequestDelete"]])(
      "does not throw error if %s is not provided",
      (prop) => {
        // Act
  ...
  describe("Modal Structure and Content", () => {
    it("should have a heading with device name and type", () => {
      ...
      // Assert
      expect(
        screen.getByRole("heading", { name: /front door camera — camera/i }),
      ).toBeInTheDocument();
    });
  ...
  describe("Button Accessibility", () => {
    it.each([["delete"]])('%s button should have type="button"', (name) => {
      // Act
      renderModal();
  ...
  describe("Accessibility", () => {
    ...
    it("image is not draggable", () => {
      ...
      renderModal();
      // Assert
      expect(screen.getByAltText("Camera feed")).toHaveAttribute(
        "draggable",
        "false",
      );
    });
  ...
  describe("Different Device Names", () => {
    it("displays long device names correctly", () => {
      ...
      renderModal({
        device: {
          ...mockDevice,
          deviceName: "Very Long Camera Name That Should Still Display",
        },
      });
      // Assert
      expect(
        screen.getByText(
          /very long camera name that should still display — camera/i,
        ),
      ).toBeInTheDocument();
    });

...
class HomeTest {
    ...
    void nameValidation_ShouldEnforceConstraints() {
        ...
        assertTrue(violations.stream().anyMatch(v -> v.getMessage().contains("Home name cannot be blank")));
        
        // Test name too long
        home.setName("a".repeat(101));
        violations = validator.validate(home);
        assertFalse(violations.isEmpty());
        assertTrue(violations.stream().anyMatch(v -> v.getMessage().contains("Home name must be between 1 and 100 characters")));
    }
    ...
    void addressValidation_ShouldEnforceConstraints() {
        ...
        violations = validator.validate(home);
        assertFalse(violations.isEmpty());
        assertTrue(violations.stream().anyMatch(v -> v.getMessage().contains("Address cannot exceed 255 characters")));
    }

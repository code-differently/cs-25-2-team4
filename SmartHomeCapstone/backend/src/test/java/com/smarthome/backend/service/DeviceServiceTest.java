...
class DeviceServiceTest {
    ...
    void createDevice_ValidThermostatRequest_ReturnsThermostat() {
        when(roomRepository.findById(1L)).thenReturn(Optional.of(testRoom));
        when(deviceRepository.existsByDeviceNameAndRoom_RoomId("Test Thermostat", 1L)).thenReturn(false);
    ...
    void createDevice_ValidCameraRequest_ReturnsCamera() {
        when(roomRepository.findById(1L)).thenReturn(Optional.of(testRoom));
        when(deviceRepository.existsByDeviceNameAndRoom_RoomId("Test Camera", 1L)).thenReturn(false);
    ...
    void createDevice_DuplicateDeviceName_ThrowsException() {
        ...
        assertEquals("Device with name 'Test Light' already exists in this room", exception.getMessage());
        verify(deviceRepository, never()).save(any());
    }
    ...
    void createDevice_UnsupportedDeviceType_ThrowsException() {
        ...
        when(roomRepository.findById(1L)).thenReturn(Optional.of(testRoom));
        when(deviceRepository.existsByDeviceNameAndRoom_RoomId("Test Device", 1L)).thenReturn(false);

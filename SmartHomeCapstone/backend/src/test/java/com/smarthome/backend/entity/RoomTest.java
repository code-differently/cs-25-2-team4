...
class RoomTest {
    ...
    void homeValidation_ShouldEnforceConstraints() {
        ...
        assertTrue(violations.stream().anyMatch(v -> v.getMessage().contains("Home cannot be null")));
    }
    ...
    void nameValidation_ShouldEnforceConstraints() {
        ...
        Set<ConstraintViolation<Room>> violations = validator.validate(room);
        assertFalse(violations.isEmpty());
        assertTrue(violations.stream().anyMatch(v -> v.getMessage().contains("Room name cannot be blank")));
        
        // Test name too long
        room.setName("a".repeat(101));
        violations = validator.validate(room);
        assertFalse(violations.isEmpty());
        assertTrue(violations.stream().anyMatch(v -> v.getMessage().contains("Room name must be between 1 and 100 characters")));
    }
